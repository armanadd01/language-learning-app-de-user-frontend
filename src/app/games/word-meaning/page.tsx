...
export default function WordMeaningGame() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true)
        setError(null)
        const res = await apiFetch<GameResponse>('/games/word-meaning?count=6')
        setGame(res.game)
        setSelections({})
        setResult(null)
      } catch (e) {
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await apiFetch<SubmitResponse>('/games/word-meaning/submit', {
        method: 'POST',
    ...
    <AppShell>
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Word Meaning Match</div>
          <div className="mt-1 text-sm font-bold text-foreground">Match each word to the correct definition</div>
        </div>
      ...
      <div className="mt-6 space-y-4">
        {game.items.map((it) => (
          <Card key={it.index} className="p-5">
            <div className="text-sm font-bold text-foreground">{it.word}</div>
