...
const learningPath: NavItem[] = [
  ...
  { label: 'Grammar', href: '/grammar', icon: <BookOpen className="h-4 w-4" /> },
]

const practice: NavItem[] = [
  { label: 'Game Hub', href: '/games', icon: <Gamepad2 className="h-4 w-4" /> },
  { label: 'My Activity', href: '/activity', icon: <Activity className="h-4 w-4" /> },
]
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    async function load() {
      if (!hasToken) return
      try {
        const res = await apiFetch<{ stats: { xpTotal: number; streakDays: number }; metrics?: { xpToday?: number } }>(
          '/me/summary'
        )
        setStats(res.stats)
      ...
      <div className="mx-auto flex w-full max-w-7xl">
        <aside className="hidden w-64 shrink-0 border-r border-border bg-[var(--card)] px-4 py-6 lg:block">
          ...
          <div className="mt-8 space-y-6">
            <div>
              <div className="px-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Learning Path</div>
            ...
            <div>
              <div className="px-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Practice</div>
              <nav className="mt-2 space-y-1">
                {practice.map((item) => {
                  const active = pathname === item.href
                  return (
            ...
            <div>
              <div className="px-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Current Goal</div>
              <div className="mt-3 rounded-2xl border border-border bg-[var(--card)] p-3">
                ...
                <div className="mt-3 h-2 w-full rounded-full bg-[var(--muted)]">
                  <div
                    ...
                    style={{ width: `${Math.min(100, Math.round(((xpToday ?? 0) / 500) * 100))}%` }}
                  />
                </div>
                <div className="mt-2 text-[11px] text-muted-foreground">{xpToday ?? 0} / 500 XP earned</div>
              </div>
            </div>
          </div>
            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
              <div className="flex items-center gap-6">
                ...
                <nav className="hidden items-center gap-4 text-sm text-muted-foreground md:flex">
                  <Link href="/dashboard" className={cn('hover:text-foreground', pathname === '/dashboard' && 'text-foreground')}>Dashboard</Link>
                  <Link href="/levels" className={cn('hover:text-foreground', pathname === '/levels' && 'text-foreground')}>Levels</Link>
                  <Link href="/grammar" className={cn('hover:text-foreground', pathname === '/grammar' && 'text-foreground')}>Grammar</Link>
                  <Link href="/games" className={cn('hover:text-foreground', pathname?.startsWith('/games') && 'text-foreground')}>Games</Link>
                  <Link href="/activity" className={cn('hover:text-foreground', pathname === '/activity' && 'text-foreground')}>Activity</Link>
                </nav>
              </div>

              <div className="flex items-center gap-3">
                <ThemeControls />
                <div className="rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700 dark:bg-orange-950/30 dark:text-orange-200">
                  🔥 {stats?.streakDays ?? 0}
                </div>
                <div className="rounded-full bg-[var(--accent)] px-3 py-1 text-xs font-semibold text-[var(--accent-foreground)]">
                  ⭐ {stats?.xpTotal ?? 0} XP
                </div>
